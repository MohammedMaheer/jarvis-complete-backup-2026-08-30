...
async def verify_voice_profile(
    ...
    try:
        result = recognize_speaker(
            ...
            household_id=household_id,
            member_ids=[user_id],
            # Keep profile verification on the same duration-aware threshold
            # policy used by live login. A second fixed threshold here caused
            # valid operators to pass runtime identification but fail the
            # enrollment verification screen with the same recording.
            threshold=None,
        )
    finally:
        os.unlink(tmp_path)
