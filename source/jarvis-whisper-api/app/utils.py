...
def _pick_threshold(duration_s: float) -> float:
    ...
    from app.services.settings_service import get_settings_service
    svc = get_settings_service()
    normal = svc.get_float("voice.similarity_threshold", 0.4)
    short = svc.get_float("voice.threshold_short", 0.65)
    long_ = svc.get_float("voice.threshold_long", 0.34)
...
def speaker_recognition_status() -> dict[str, object]:
    ...
    return {
        ...
        "thresholds": {
            "short": svc.get_float("voice.threshold_short", 0.65),
            "normal": svc.get_float("voice.similarity_threshold", 0.4),
            "long": svc.get_float("voice.threshold_long", 0.34),
            "short_cutoff_seconds": svc.get_float("voice.short_cutoff_seconds", 1.0),
            "long_cutoff_seconds": svc.get_float("voice.long_cutoff_seconds", 3.0),
        },
