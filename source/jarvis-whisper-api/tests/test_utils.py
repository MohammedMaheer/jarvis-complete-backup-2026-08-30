...
class TestCachedProfileCounts:
    ...
    def test_status_exposes_active_thresholds_for_diagnostics(self, monkeypatch) -> None:
        """Operators can detect threshold drift without inspecting the DB."""
        from app import utils

        values = {
            "voice.threshold_short": 0.65,
            "voice.similarity_threshold": 0.4,
            "voice.threshold_long": 0.34,
        ...
        status = utils.speaker_recognition_status()
        assert status["thresholds"] == {
            "short": 0.65,
            "normal": 0.4,
            "long": 0.34,
            "short_cutoff_seconds": 1.0,
            "long_cutoff_seconds": 3.0,
        }
