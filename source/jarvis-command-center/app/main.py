...
from app.deps import verify_api_key, get_model_service, enforce_secret_security, require_app_auth
...
from app.core.ui_events import ui_event_broker, sse_frame, publish_ui_event
...
@v0_router.get("/ui/events")
async def ui_events(request: Request, _: None = Depends(require_app_auth)):
    """Authenticated, redacted lifecycle events for the local desktop deck."""

    async def event_stream() -> AsyncIterator[str]:
        try:
            async for event in ui_event_broker.subscribe(request.headers.get("last-event-id")):
                if event is None:
                    yield ": heartbeat\n\n"
                else:
...
async def handle_voice(
    ...
    await publish_ui_event(
        "thinking", "start", "llm", "Processing local command",
        conversation_id=request.conversation_id,
    )
    ...
    try:
        ...
        await publish_ui_event(
            "memory", "progress", "memory", "Loading scoped household context",
            conversation_id=request.conversation_id,
        )
        await publish_ui_event(
            "executing", "start", "tool", "Routing command through local agent graph",
            conversation_id=request.conversation_id,
        )
        ...
        await publish_ui_event(
            "executing", "complete", "tool", "Local agent graph complete",
            conversation_id=request.conversation_id,
            metadata={"toolCount": len(result.get("tool_calls", []))},
        )
        ...
        latency_logger.end_request(request.conversation_id)
        await publish_ui_event(
            "complete", "complete", "system", "Command complete",
            conversation_id=request.conversation_id,
        ...
        latency_logger.end_request(request.conversation_id)
        await publish_ui_event(
            "warning", "error", "system", "Command was rejected",
            conversation_id=request.conversation_id,
        )
        raise
        ...
        latency_logger.end_request(request.conversation_id)
        await publish_ui_event(
            "warning", "error", "system", "Conversation is not ready",
            conversation_id=request.conversation_id,
        )
        ...
        latency_logger.end_request(request.conversation_id)
        await publish_ui_event(
            "error", "error", "system", "Local command failed",
            conversation_id=request.conversation_id,
            latency_ms=round((time.time() - start_time) * 1000),
        )
        duration = time.time() - start_time
...
async def _end_trace_after_stream(
    ...
    first_chunk = True
    await publish_ui_event(
        "speaking", "start", "tts", "Local speech synthesis started",
        conversation_id=conversation_id,
    )
    try:
        async for chunk in stream:
            if first_chunk:
                first_chunk = False
                await publish_ui_event(
                    "speaking", "progress", "tts", "First audio frame ready",
                    conversation_id=conversation_id,
                )
        ...
        await publish_ui_event(
            "complete", "complete", "tts", "Speech output complete",
            conversation_id=conversation_id,
        )
        if timing is not None:
            timing.record_span(
...
async def handle_voice_stream(
    ...
    start_time = time.time()
    await publish_ui_event(
        "thinking", "start", "llm", "Streaming local response",
        conversation_id=request.conversation_id,
    )
    ...
    try:
        ...
        if streaming_audio is not None:
            await publish_ui_event(
                "thinking", "complete", "llm", "Local response ready",
                conversation_id=request.conversation_id,
            )
            try:
        ...
        if streaming_audio is not None:
            await publish_ui_event(
                "executing", "complete", "tool", "Tool route complete",
                conversation_id=request.conversation_id,
            )
            try:
                audio_fmt = await tts_client.get_audio_format()
        ...
        latency_logger.end_request(request.conversation_id)
        await publish_ui_event(
            "error", "error", "system", "Streaming command failed",
            conversation_id=request.conversation_id,
            latency_ms=round((time.time() - start_time) * 1000),
        )
        duration = time.time() - start_time
