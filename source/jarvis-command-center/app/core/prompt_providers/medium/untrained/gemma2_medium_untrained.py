...
# Gemma 2 frequently ignores the <tool_call> tag instruction and instead wraps
# the call in a markdown code fence — ```tool_call{...}```, ```json{...}```, or a
# bare ```{...}```. Capture the fence CONTENT (not a brace-delimited group, which
# would mis-split on the nested "arguments" object) and let json.loads parse it.
_FENCED_TOOL_CALL_RE = re.compile(
...
"""
    ...
    def parse_response(self, raw_content: str) -> Optional[str]:
        ...
        # Fallback: Gemma 2 often emits the call in a markdown code fence
        # (```tool_call{...}``` / ```json{...}```) instead of <tool_call> tags.
        fenced_calls: list[Dict[str, Any]] = []
        for match in _FENCED_TOOL_CALL_RE.findall(cleaned):
            try:
                call_obj = json.loads(match.strip())
            except json.JSONDecodeError:
                continue
            if not isinstance(call_obj, dict) or "name" not in call_obj:
                continue  # a fenced JSON blob that isn't a tool call
            arguments = call_obj.get("arguments", {})
            if isinstance(arguments, dict):
                for key in _ARRAY_PARAMS:
                    if key in arguments and isinstance(arguments[key], str):
                        arguments[key] = [arguments[key]]
            fenced_calls.append(call_obj)
        if fenced_calls:
            return json.dumps({"message": "", "tool_calls": fenced_calls, "error": None})
